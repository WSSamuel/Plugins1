 ...
 */
add_action( 'show_user_profile', 'pucsi_logins_count_field_callback', 15 );
add_action( 'edit_user_profile', 'pucsi_logins_count_field_callback', 15 );
 ...
 */
add_action( 'personal_options_update', 'pucsi_update_logins_count_field_callback', 10 );
add_action( 'edit_user_profile_update', 'pucsi_update_logins_count_field_callback', 10 );
...
// Replace the existing code with the modified code
...
// using the "authenticate" filter hook,
// if it is more, do not allow them to log in and show the error message on the login screen
function pucsi_auth_signon_callback( $user, $username, $password ) {
    // Check whether the username exists; if not, return the user as is
    if (!empty($username)) {
        ...
        // Get user sessions
        $user_sessions = get_user_meta($user_id, 'session_tokens', true);
        // Get login timestamps from all sessions
        $login_timestamps = (!empty($user_sessions)) ? array_values(wp_list_pluck($user_sessions, 'login')) : array();
        // Check whether the user's session is less than the limit specified by the administrator; if so, continue
        if (count($login_timestamps) <= $_login_count) {
            // Return the user
            return $user;
        } else {
            // Allow users to customize or extend functionality by providing the following action hooks
            ...
            // Message
            $message = sprintf(
                esc_html__('Max %s login sessions are allowed at a time. Please contact your site administrator for more details.', 'pucsi'),
                $_login_count
            );
            // Set up an error message filter to allow users to customize error messages when they need to use filters
            $message = apply_filters('pucsi_restrict_number_of_sign_ins_error_message', $message, count($login_timestamps), $_login_count);
            // Return an error message
            wp_die($message, esc_html__('Login Error', 'pucsi'), array('response' => 403));
        }
    }
    // Return the user
    return $user;
