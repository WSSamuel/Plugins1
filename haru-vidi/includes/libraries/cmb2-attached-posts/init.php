...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function render( $field, $escaped_value, $object_id, $object_type, $field_type ) {
		...
		if ( $this->field->options( 'filter_boxes' ) ) {
			$filter_boxes = '<div class="search-wrap"><input type="text" placeholder="' . sprintf( __( 'Filter %s', 'haru-vidi' ), $post_type_labels ) . '" class="regular-text search" name="%s" /></div>';
		}
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'Available %s', 'haru-vidi' ), $post_type_labels ) . '</h4>';
		...
		if ( ! $query_users ) {
			$findtxt = $field_type->_text( 'find_text', __( 'Search', 'haru-vidi' ) );
		...
		echo '<h4 class="attached-posts-section">' . sprintf( __( 'Attached %s', 'haru-vidi' ), $post_type_labels ) . '</h4>';
		...
		echo $field_type->input( array(
			'name'	=> $this->field->_id() . '-current',
			'id'	=> $this->field->_id() . '-current',
			'type'  => 'hidden',
			'class' => 'attached-posts-ids-current',
			'value' => ! empty( $ids ) ? implode( ',', $ids ) : '',
			'desc'  => '',
		) );
