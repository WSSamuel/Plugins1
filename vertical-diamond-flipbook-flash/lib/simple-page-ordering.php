...
class nfb_Simple_Page_Ordering {
	...
	public function load_edit_screen() {
		...
		$sortable = true;//( post_type_supports( $screen->post_type, 'page-attributes' ) || is_post_type_hierarchical( $screen->post_type ) );		// check permission
		$sortable = apply_filters( 'simple_page_ordering_is_sortable', $sortable, $screen->post_type );
		
		if ( ! $sortable || $screen->post_type != NFB_FLIPBOOK_POST_TYPE  )
			return;
	...
	public function wp() {
		if ( strpos( get_query_var('orderby'), 'menu_order' ) === 0 ) {	// we can only sort if we're organized by menu order
			...
			wp_enqueue_script( 'simple-page-ordering', NFB_PLUGIN_URL.'/js/'.$script_name, array('jquery-ui-sortable'), '2.0', true );
		}
	}
...
$simple_page_ordering = new nfb_Simple_Page_Ordering;