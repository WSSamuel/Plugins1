=== Convertix Pixel ===

Contributors: convertix
Tags: convertix,facebook,facebook-conversion-api,tiktok,google-analytics,google-adwords,instagram,conversion,conversion-api,adwords,analytics,server-side,container,tiktokads,
Requires at least: 6.0.1
Tested up to: 6.1.1
Stable tag: 1.4.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

== Description ==

Convertix Pixel Integration.
Please, be advised that **this plugin will not work properly without a valid account** with Convertix Company.

> First, create an account at [https://convertix.digital/](https://convertix.digital/?utm_source=plugin-wordpress "Convertix Digital Marketing")

== This plugin lets you: ==

* Add Convertix Pixel to your website.
* If you have an existing setup of GTM and other Pixel using another plugin or hardcoded, you'll need to remove it for conflicts.
* Automatically tracks PageView events without js library and optimize page speed.
* Track your website's visitors without the hasle of inserting pixels code and not seeing all convertions in your ads platform.

== Convertix Digital Pixel makes your analytics data resistant to: ==

* 3rd party cookies blockers (adBlockers can't block it)
* intelligent tracking protection (Safari, Firefox, etc will track all users)
* Optimize you ad spending with Advanced Matching Tracking

== Installation ==

1. Unzip and upload the "convertix-pixel" to the '/wp-content/plugins/' directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Go to Settings / Convertix Pixel, enter your informations and set additional options

== Frequently Asked Questions ==

= What is Convertix =

Please refer to the description of <a href="https://convertix.digital/">Convertix Digital Marketing</a>.

= Which Marketing Platforms Does Convertix Integrates to =

Meta Pixel (Facebook and Instagram), Google AdWords, Google Analytics 4, Tiktok Pixel, LinkedIn Insight Tag, ActiveCampaign Pixel, and much more to come.

= Can I use Convertix Pixel Without an Accont? =

No, for now you need an account to use the plugin.

== Screenshots ==

1. Plugin's Support Base.
2. Menu item in the settings panel.
3. Plugin's Settings Page
4. Activated plugin in plugin's page

== Changelog ==

= 1.0.0 =
* Initial release.
+ Includes Meta and Google Pixel
+ Includes Convertix Pixel

= 1.0.1 - 1.0.16 =
+ Good name fixes for publishing plugin on wordpress.org
* Fix input names to match variables name changes

= 1.1 =
+ Included Tiktok Pixel
+ Included LinkedIn Pixel
* Automatic calculation for Convertix Variable (disabled)
* Solved issue with interface

= 1.2 =
+ Included On/Off Plugin
* Off Code only for testing purposes

= 1.3 =
+ Included ActiveCampaign Tracking Code
* Internal perfomance improvements
* Fix internal variables
* Perfomance issues

= 1.4.1 =
+ Fix issue to Google Optimize within Plugin

= 1.4.0 =
+ Included support to Google Optimize within Plugin
* fixed bug on FB Pixel API 2
* Performance Issues with GTM Client Variables

== Upgrade Notice ==
= 1.4.1 =
Use this last version for full support and stable performance