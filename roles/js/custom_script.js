( function ( $ ) {
    ...
    $.validator.addMethod( "role_exist", function ( value, element ) {
        var icreon_wp_roles_obj = jQuery.parseJSON( icreon_wp_roles.icreon_wp_roles );
        if ( value.trim() == 'administrator' || typeof icreon_wp_roles_obj[value.trim()] !== 'undefined' ) {
            return false;
        }
        return true;
    ...
    $.validator.addMethod( "role_name_exist", function ( value, element ) {
        ...
        var status = true;
        $.each( jQuery.parseJSON( icreon_wp_roles.icreon_wp_roles ), function ( index, existing_value ) {
    ...
    $( document ).ready( function () {
        ...
        if ( $( "table.icreon-rnc-table-head" ).length > 0 ) {
            $( '[data-toggle="tooltip"]' ).tooltip();
            $( "table.icreon-rnc-table-head" ).stickyTableHeaders();
        }
        var $uniformed;
        $uniformed = $( "#icreon_capability_form tbody" ).find( "input" ).not( ".skipThese" );
        if ( $uniformed.length ) {
            $uniformed.uniform();
        }
        ...
        $( '#icreon_add_capability_form' ).validate( {
            rules: {
                'cap_name': {
                    minlength: 3,
                    required: true,
        ...
        $( "#icreon_export_capability" ).validate( {
            rules: {
                cap_role_export: {
                    select_one: true
                }
