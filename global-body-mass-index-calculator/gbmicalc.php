...
$gmbi_inst = new GBMI_Calc_Widget();
register_activation_hook(__FILE__, array($gmbi_inst, 'activate'));
register_deactivation_hook(__FILE__, array($gmbi_inst, 'deactivate'));
...
class GBMI_Calc_Widget {
    ...
    function control() {
        ...
        if (isset($_POST['title'])) { 
            ...
            $data['standard'] = esc_attr($_POST['standard']);
            $data['height'] = esc_attr($_POST['height']);
            $data['width'] = esc_attr($_POST['width']);
            $data['bgcolor'] = esc_attr($_POST['bgcolor']);
            $data['bgendcolor'] = esc_attr($_POST['bgendcolor']);
            $data['textcolor'] = esc_attr($_POST['textcolor']);
            $data['allowLink'] = esc_attr($_POST['allowLink']);
            update_option('GBMI_Calc_Widget', $data);
        }
    }
    ...
    function widget($args, $content=NULL) {
        ...
        <?php if ($standard == 'standard') { ?>
                            <div>
                                ...
                                <input name="bmifeet" class="input_fw"/> <?php _e('ft', MY_TEXTDOMAIN); ?> <input name="bmiinches" style="width: 40px;"/> in
                            </div>
                            <div>
    ...
    function register() {
        wp_register_sidebar_widget('GBMI Calculator', "GBMI Calculator Widget", array($this, 'widget'));
        wp_register_widget_control('GBMI Calculator', "GBMI Calculator Widget", array($this, 'control'));
    }
}

// class closed

add_shortcode('gbmicalc', array($gmbi_inst, 'widget'));
add_action('widgets_init', array($gmbi_inst, 'register'));
...
function gbmi_calc_settings() {
  ...
  <br/>
    ...
    if (isset($_POST['submit'])) {
        ...
        $data['textcolor'] = esc_attr($_POST['textcolor']);
        $data['allowLink'] = esc_attr($_POST['allowLink']);
        $data['tabcolor'] = esc_attr($_POST['tabcolor']);
        $data['currtabcolor'] = esc_attr($_POST['currtabcolor']); 
        update_option('GBMI_Calc_Widget', $data); 
